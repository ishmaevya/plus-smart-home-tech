...
public class AggregatorServiceImpl {
    ...
    public Optional<SensorsSnapshotAvro> updateState(SensorEventAvro event) {
        ...
        if (!isUpdateNeeded(snapshot.getSensorsState(), event, sensorState)) {
        ...
        SensorsSnapshotAvro updatedSnapshot = SensorsSnapshotAvro.newBuilder(snapshot)
                ...
                .setTimestamp(sensorState.getTimestamp())
                .build();
    ...
    private boolean isUpdateNeeded(Map<String, SensorStateAvro> sensorsState, SensorEventAvro event,
                                   SensorStateAvro sensorState) {
        SensorStateAvro existing = sensorsState.get(event.getId());
        if (existing == null) {
            return true;
        }
        if (existing.getData().equals(sensorState.getData())) {
            return false;
        }
        return !existing.getTimestamp().truncatedTo(ChronoUnit.MILLIS).isAfter(event.getTimestamp());
    }
